...
COPY --from=builder /workspace/build/libs/*.jar config-service-app.jar

# 파일 소유권을 non-root 사용자로 변경
RUN chown ${USERNAME}:${USERNAME} config-service-app.jar
...
# 컨테이너 실행 명령어
ENTRYPOINT ["java", "-jar", "config-service-app.jar"]